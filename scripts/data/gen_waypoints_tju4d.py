...
def compute_gps_heading(gnss, lat0, lon0):
    """
    Compute heading from GPS trajectory.
    ...
    for key in HEADING_KEYS:
        ...
        east, north = gps_to_enu(lat0, lon0,
...
def compute_heading_from_neighbors(all_gnss, idx, max_look=10):
    """
...
def process_sequence(seq_path, output_subdir="WAYPOINTS"):
    ...
    for i, gnss_data in enumerate(tqdm(all_gnss, desc=f"  {seq_name}")):
        ...
        heading_ned, heading_src = compute_gps_heading(gnss, lat0, lon0)

        # Fallback: use neighboring frames
        if heading_ned is None:
            heading_ned = compute_heading_from_neighbors(all_gnss, i)
